# WinRayCast Editor Plan

## Purpose

The editor should make WinRayCast content authoring practical before the engine
...
tools/
  WinRaycastEditor/
    WinRaycastEditor.sln
    src/
      WinRaycastEditor/
      WinRaycastEditor.Core/
      WinRaycastEditor.Tests/
```
...
For WinRayCast this means the cell remains the canonical editable unit, but it
...
- Create WPF solution under `tools/WinRaycastEditor`.
...
- Update WinRayCast demo to optionally load the exported scene description.
...
- Created `tools/WinRaycastEditor` solution.
- Added WPF shell project targeting .NET 8 Windows.
- Added `WinRaycastEditor.Core` for editor-side models and parsing.
- Added `WinRaycastEditor.Tests` with MSTest coverage.
...
- Added a native C++ `SceneLoader` that parses the editor project file format
  in the WinRayCast engine library, with gtest coverage for valid projects,
  defaults, missing files, and invalid JSON.
- Extended the WinRayCast demo to accept an optional project JSON path on the
...
- Added a WinRayCast demo menu command (`File > Open Project...`) that lets the
...
- Kept the command-line project loading path intact and added a WPF `Test 3D`
  ...
  WinRayCast demo executable, and launches the engine with that project path so
...
- Hooked the engine demo (`WinRayCast.cpp`) up so a project pointing at a
...
- New variable-height demo world `res/demo.world.json` plus a thin
  `res/demo.winrayproj.json` project that references it. The world ships in
...
- Added a CMake custom target `WinRaycastEditorPublish` that runs
  `dotnet publish` for `WinRaycastEditor.csproj` with
  `-r win-x64 --self-contained -p:PublishSingleFile=true`. The output ends up
  in `<build>/editor_publish/WinRaycastEditor.exe` and is consumed by the
...
- Integrated the editor solution into the CMake-generated Visual Studio
  ...
  `WinRaycastEditor.Core.csproj`, `WinRaycastEditor.csproj`, and
  `WinRaycastEditor.Tests.csproj` as native `Any CPU` projects inside the
  generated `WinRayCast.sln`/`.slnx`, organized under an "Editor" folder
...
- Result of `cpack -G ZIP`: a 33-entry archive with `WinRayCast.exe`,
  `editor/WinRaycastEditor.exe`, `res/` (textures, world.ini, demo world JSON,
...
3. Package: `cpack --config out/build/vs2026-x64/CPackConfig.cmake -C Release`.
   With NSIS installed, this produces `WinRayCast-<ver>-win-x64-setup.exe`
...
(`WinRaycastEditor.Core` + WPF host + MSTest suite of ~20 test files) built as
