...
public static class EditorSceneExporter
{
    ...
    private static string CopyRuntime(string engineExecutablePath, string projectDirectory)
    {
        if (!File.Exists(engineExecutablePath)) {
            throw new FileNotFoundException("Missing nuRCADE Player executable.", engineExecutablePath);
        }

        var destinationPath = Path.Combine(projectDirectory, "nuRCADEPlayer.exe");
        if (!string.Equals(
    ...
    private static string WriteRunScript(string projectDirectory, string projectFileName)
    {
        ...
        File.WriteAllText(
            ...
            string.Join(
                ...
                "setlocal",
                "pushd \"%~dp0\"",
                "nuRCADEPlayer.exe \"%~dp0" + projectFileName + "\"",
