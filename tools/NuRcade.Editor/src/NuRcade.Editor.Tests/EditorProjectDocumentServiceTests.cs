...
public sealed class EditorProjectDocumentServiceTests
{
    ...
    {
        var directory = Directory.CreateTempSubdirectory("nurcade-editor-project-");
        var path = Path.Combine(directory.FullName, "demo.nurcadeproj.json");
    ...
    {
        var directory = Directory.CreateTempSubdirectory("nurcade-editor-project-");
        var path = Path.Combine(directory.FullName, "legacy.nurcadeproj.json");
        try {
    ...
    {
        var sourceDirectory = Directory.CreateTempSubdirectory("nurcade-editor-source-");
        var exportDirectory = Directory.CreateTempSubdirectory("nurcade-editor-export-");
        var sourceWorldPath = Path.Combine(sourceDirectory.FullName, "world.world.json");
        var projectPath = Path.Combine(exportDirectory.FullName, "demo.nurcadeproj.json");
    ...
    public void SceneExporterCanWritePlayableDemoPackage()
    {
        var sourceDirectory = Directory.CreateTempSubdirectory("nurcade-editor-source-");
        var exportDirectory = Directory.CreateTempSubdirectory("nurcade-editor-export-");
        var runtimeDirectory = Directory.CreateTempSubdirectory("nurcade-editor-runtime-");
        var sourceWorldPath = Path.Combine(sourceDirectory.FullName, "world.world.json");
        var projectPath = Path.Combine(exportDirectory.FullName, "demo.nurcadeproj.json");
        var enginePath = Path.Combine(runtimeDirectory.FullName, "nuRCADE.exe");
        ...
        try {
            ...
            Assert.IsTrue(File.Exists(Path.Combine(exportDirectory.FullName, "nuRCADEPlayer.exe")));
            Assert.IsTrue(File.Exists(Path.Combine(exportDirectory.FullName, "run_demo.bat")));
            StringAssert.Contains(File.ReadAllText(result.RunScriptPath!), "nuRCADEPlayer.exe \"%~dp0demo.nurcadeproj.json\"");
