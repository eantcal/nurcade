...
{
    private const string StartMenuFolderName = "nuRCADE Demos";
        ...
        string displayName)
    {
        if (!File.Exists(projectPath)) {
            throw new FileNotFoundException("Missing exported nuRCADE project.", projectPath);
        }

        if (!File.Exists(enginePath)) {
            throw new FileNotFoundException("Missing exported nuRCADE runtime.", enginePath);
        }
