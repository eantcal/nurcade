...
public:
	...
	// World-configured text for the bottom teletype event log. Templates may contain
	// {name} (item name) and {amount} (e.g. healed hit points) placeholders.
	struct MessageLogConfig {
		bool enabled = true;
		int maxLines = 4;
		std::string keyPickup = "Acquired {name} key";
		std::string ammoPickup = "Ammunition replenished";
		std::string healthPickup = "Health restored (+{amount})";
		std::string mapUnlocked = "Map online";
		std::string mapActorsUnlocked = "Enemy tracking online";
		std::string weaponReload = "Reloading {name}";
		std::string itemPickup = "Acquired {name}";
	};

	WorldMap() = default;

	const MessageLogConfig& messageLog() const noexcept {
		return m_messageLog;
	}

	MessageLogConfig& messageLog() noexcept {
		return m_messageLog;
	}
	...
	bool hasPlayerStart() const noexcept {
		return m_hasPlayerStart;
	}
	...
	// Overrides the progressive-turn feel. Non-positive arguments leave the current
	// value untouched, so a world can specify only the fields it cares about. The cap
	// is kept at least as large as the base rate.
	void setPlayerTurnConfig(
		double baseDegPerSec,
		double maxDegPerSec,
	...
	void setPlayerStartCell(double xCell, double yCell, double facingDegrees) noexcept {
	...
	void applyTexture(
		MapCell::TextureResourceKey textureKey,
		std::shared_ptr<Texture> texture) noexcept {
	...
	void setPlayerInteriorOverride(MapCell::TextureKey key, int row, int column) noexcept
	{
	...
	bool usesBlockLayout() const noexcept {
		return m_hasBlockIds;
	}
