...
namespace {
	...
	bool doorKeyringContains(
		const std::vector<std::string>& keyring,
		...
		return std::find_if(
			...
			[&normalizedRequired](const std::string& key) {
				...
				return normalizedKey == normalizedRequired
					|| normalizedKey.find(normalizedRequired) != std::string::npos
					|| normalizedRequired.find(normalizedKey) != std::string::npos;
			}) != keyring.end();
	}
		...
		const std::vector<std::string>& keyring) noexcept
	{
		...
		const auto exact = door.lockedOverlayTextureKeysByKey.find(normalizedRequired);
		...
		double timeSeconds) noexcept
	{
		...
		auto frameIndex = static_cast<size_t>(
			std::floor(std::max(0.0, timeSeconds) / animation.frameDurationSeconds));
		if (animation.loop) {
		...
		double timeSeconds) noexcept
	{
		...
		return allFaces != nullptr
			? textureAnimationFrame(*allFaces, timeSeconds)
			: 0;
	}
...
bool WorldMap::setMapInfo(const Cell* array, uint32_t rows, uint32_t cols)
{
	...
	m_doorStates.assign(
		rows,
		std::vector<DoorRuntimeState>(cols));
...
double WorldMap::doorOpenAmountAt(int row, int column) const noexcept
{
	if (row < 0
		|| column < 0
		|| row >= static_cast<int>(m_doorStates.size())
	...
	bool internalWall) const noexcept
{
	...
	if (block != nullptr
		...
		const auto amount = std::max(0.0, std::min(1.0, doorOpenAmountAt(row, column)));
...
void WorldMap::advanceDynamicTextures(double deltaSeconds) noexcept
{
	...
	if (m_dynamicTextureTimeSeconds > 86400.0) {
		m_dynamicTextureTimeSeconds = std::fmod(m_dynamicTextureTimeSeconds, 86400.0);
	}
}
...
WorldMap::TextureLayer WorldMap::transparentWallTextureLayerAt(
	int row,
	int column,
...
MapCell::TextureKey WorldMap::transparentWallTextureAt(
	int row,
	int column,
	MapCell::TextureKey fallback,
	WinRayCast::WallFace face,
...
void WorldMap::updateDoors(
	double playerX,
	double playerY,
	const std::vector<Point2d>& actorPositions,
	double deltaSeconds,
	std::vector<DoorEvent>* events) noexcept
{
	...
	const auto cellSize =
		(static_cast<double>(getCellDx()) + static_cast<double>(getCellDy())) * 0.5;
	...
	for (int row = 0; row < getRowCount(); ++row) {
		for (int column = 0; column < getColCount(); ++column) {
			...
			const auto triggerDistance = std::max(0.0, block->door.triggerDistanceCells) * cellSize;
			const auto doorX = (static_cast<double>(column) + 0.5) * getCellDx();
			...
			if (!block->door.requiredKey.empty()) {
				isNear = playerIsNear
					&& doorKeyringContains(m_doorKeyring, block->door.requiredKey);
			}
			else {
				isNear = playerIsNear;
			...
			if (isNear) {
				...
				state.openAmount = std::min(1.0, state.openAmount + deltaSeconds / openTime);
				if (events != nullptr
					&& previousOpenAmount <= 0.0
					&& state.openAmount > 0.0) {
					events->push_back({
						DoorEvent::Type::OpeningStarted,
						row,
						column,
						blockIdAt(row, column)
						});
				}
			}
			else if (state.closeDelay > 0.0) {
				state.closeDelay = std::max(0.0, state.closeDelay - deltaSeconds);
			}
			else {
				state.openAmount = std::max(0.0, state.openAmount - deltaSeconds / openTime);
			}
		}
	}
}
...
void WorldMap::forceDoorClosingAt(int row, int column, double deltaSeconds) noexcept
{
	if (!m_hasBlockIds || deltaSeconds <= 0.0) {
		return;
	}
