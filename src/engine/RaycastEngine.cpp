...
void parallelColumns(int total, Fn&& fn)
{
    if (total <= 1) {
        fn(0, total);
        return;
...
{
	return colorRed(lhs) == colorRed(rhs)
		&& colorGreen(lhs) == colorGreen(rhs)
...
static bool isTransparentTexturePixel(
	...
	Color color,
	Color transparentColor) noexcept
{
	if (texture.hasAlpha()) {
...
static Color shadeWorldColor(Color color, double factor) noexcept
{
	const auto clampedFactor = std::clamp(factor, 0.0, 2.0);
	return withAlpha(
		makeColor(
			std::min(255.0, colorRed(color) * clampedFactor),
			std::min(255.0, colorGreen(color) * clampedFactor),
			std::min(255.0, colorBlue(color) * clampedFactor)),
		colorAlpha(color));
}
...
static Color alphaBlendOver(Color base, Color overlay, uint8_t alpha) noexcept
{
	...
	return withAlpha(
		makeColor(
			(static_cast<int>(colorRed(overlay)) * alpha
				+ static_cast<int>(colorRed(base)) * inverseAlpha) / 255,
			(static_cast<int>(colorGreen(overlay)) * alpha
				+ static_cast<int>(colorGreen(base)) * inverseAlpha) / 255,
			(static_cast<int>(colorBlue(overlay)) * alpha
				+ static_cast<int>(colorBlue(base)) * inverseAlpha) / 255),
		255);
}

static Color overlayWallPixel(
	Color base,
	const Texture* overlayTexture,
	int xSrc,
	int ySrc,
	bool baseIsTransparent = false,
	bool* becameOpaque = nullptr) noexcept
{
	...
	if (overlayTexture == nullptr
		|| overlayTexture->empty()
		|| overlayTexture->width() == 0
		|| overlayTexture->height() == 0) {
		return base;
	}

	const auto overlayX = static_cast<uint32_t>(
		xSrc >= 0 ? xSrc : 0) % overlayTexture->width();
	const auto overlayY = static_cast<uint32_t>(
		ySrc >= 0 ? ySrc : 0) % overlayTexture->height();
	const auto overlay = overlayTexture->getPixel(overlayX, overlayY);
	const auto alpha = overlayTexture->hasAlpha()
		? colorAlpha(overlay)
		: uint8_t(255);
	if (alpha == 0) {
		return base;
	...
	if (baseIsTransparent) {
		return withAlpha(
			makeColor(colorRed(overlay), colorGreen(overlay), colorBlue(overlay)),
			alpha);
	}
...
// The wall texture column is derived from the world X/Y of the hit, which runs in
// a fixed direction regardless of which side of the wall is being viewed. Without
// correction the two opposing faces sample the column in opposite screen order, so
// asymmetric textures (e.g. text) come out mirrored on half of the orientations.
// Mirror the column for the viewing direction that needs it, keyed purely on the
// ray direction so it is correct for inner faces too.
static bool shouldMirrorWallColumn(const Player& player, int relRay, bool vertical) noexcept
{
	if (vertical) {
...
static bool rayExitsMapAtTransparentHit(
	const WorldMap& map,
	const Player& player,
	int ray,
	bool vertical,
	const Point2d& point) noexcept
{
	...
	if (vertical) {
		const auto headingWest = ray >= player.deg90() && ray < player.deg270();
		return headingWest
			? point.first <= epsilon
			: point.first >= static_cast<double>(map.getMaxX()) - epsilon;
	}

	const auto headingNorth = ray >= player.deg180() && ray < player.deg360();
	return headingNorth
		? point.second <= epsilon
		: point.second >= static_cast<double>(map.getMaxY()) - epsilon;
}
...
void RaycastEngine::clearViewWeapon() noexcept
{
	m_viewWeapon.reset();
}

ViewWeapon* RaycastEngine::viewWeapon() noexcept
{
...
void RaycastEngine::advanceSpriteAnimations(
	double deltaSeconds,
	const std::vector<size_t>& excludedSpriteIndices) noexcept
{
	...
	for (size_t spriteIndex = 0; spriteIndex < m_sprites.size(); ++spriteIndex) {
		if (std::find(
			excludedSpriteIndices.begin(),
			excludedSpriteIndices.end(),
			spriteIndex) != excludedSpriteIndices.end()) {
			continue;
		}
	...
	const Texture* overlayTexture)
{
	...
	while (yd < max_yd && ys < height_source) {
		...
		if (depthIndex < m_opaquePixelDepthBuffer.size()) {
			m_opaquePixelDepthBuffer[depthIndex] = std::min(
		...
		drawPixel32(
			xDest,
			yd,
			shadeWorldColor(c, std::min(1.0, depthPar) * m_brightness));
	...
	const Texture* overlayTexture)
{
	...
	while (yd < max_yd && ys < height_source) {
		...
		if (!baseIsTransparent || becameOpaque) {
			...
			c = shadeWorldColor(c, std::min(1.0, depthPar) * m_brightness);
...
bool RaycastEngine::canPlaceSprite(
	const Sprite& sprite,
	double x,
	double y,
	const WorldMap& map,
	SpriteCollisionMode collisionMode) const noexcept
{
...
bool RaycastEngine::moveSprite(
	size_t spriteIndex,
	double dx,
	double dy,
	const WorldMap& map,
	SpriteCollisionMode collisionMode) noexcept
{
...
void RaycastEngine::renderSprites(WorldMap& wMap)
{
	...
	input.verticalCenterOffsetRows = static_cast<double>(m_player.getSlope()) * 0.5;
	...
	for (const auto& sprite : m_sprites) {
		...
		if (!SpriteProjector::hasVisibleColumn(projection, m_columnDepthBuffer)) {
			continue;
		}

		const auto& frames = sprite.activeFrames();
		const auto viewIndex = frames.selectViewIndex(
			input.cameraX,
			input.cameraY,
			sprite.x,
			sprite.y,
	...
	for (const auto& visible : visibleSprites) {
		...
		const auto spriteShading = std::min(
			1.0,
			(m_scale / std::max(1.0, projection.depth)) / m_depthShadingPar)
			* m_brightness;
		...
		for (int column = firstColumn; column <= lastColumn; ++column) {
			...
			const int lastRow = std::min(
				m_player.getYProjRes() - 1,
				projection.bottomRow);
			...
			for (int row = firstRow; row <= lastRow; ++row) {
				...
				auto color = texture->getPixel(textureX, textureY);
				if (isTransparentTexturePixel(
					*texture,
					color,
					visible.sprite->transparentColor)) {
					continue;
				}
				color = shadeWorldColor(color, spriteShading);
				...
				if (depthIndex < m_spritePixelDepthBuffer.size()) {
					m_spritePixelDepthBuffer[depthIndex] = std::min(
						m_spritePixelDepthBuffer[depthIndex],
						projection.depth);
				}
			}
		}
...
void RaycastEngine::renderViewWeapon()
{
	...
	const auto originX = static_cast<int>(std::round(
		static_cast<double>(screenWidth) * m_viewWeapon->anchorX()
		- static_cast<double>(destWidth) * m_viewWeapon->anchorX()
		+ m_viewWeapon->baseOffsetX()
		+ m_viewWeapon->bobOffsetX()));
	const auto originY = static_cast<int>(std::round(
		static_cast<double>(screenHeight) * m_viewWeapon->anchorY()
		- static_cast<double>(destHeight)
		+ m_viewWeapon->baseOffsetY()
	...
	for (int y = firstY; y <= lastY; ++y) {
		...
		for (int x = firstX; x <= lastX; ++x) {
			const auto textureX = static_cast<uint32_t>(
				((x - originX) * static_cast<int>(texture->width())) / destWidth);
	...
	bool render_internal_wall)
{
	...
	const auto cameraCell = std::pair<int, int>{
		m_player.getY() / wMap.getCellDy(),
	...
	for (int ray = 0; ray < m_player.getXProjRes(); ++ray) {
		...
		while ((!v_not_found || !h_not_found) && hitCount < hits.size()) {
			if (render_internal_wall) {
				while (!v_not_found
					&& (vCellVal = vertIntWall(wMap, pv, relRay), !MapCell::hasAnyWall(vCellVal))) {
				...
				while (!h_not_found
					&& (hCellVal = horzIntWall(wMap, ph, relRay), !MapCell::hasAnyWall(hCellVal))) {
				...
				while (!v_not_found
					&& (vCellVal = vertWall(wMap, pv, relRay), !MapCell::hasAnyWall(vCellVal))) {
					vertint(wMap, pv, relRay, M, pv);
					if (!isInClientRect(wMap, pv)) {
						v_not_found = true;
						break;
				...
				while (!h_not_found
					&& (hCellVal = horzWall(wMap, ph, relRay), !MapCell::hasAnyWall(hCellVal))) {
					horzint(wMap, ph, relRay, M1, ph);
					if (!isInClientRect(wMap, ph)) {
						h_not_found = true;
						break;
			...
			const double dh = h_not_found ? POSITIVE_INFINITY : horzDist(ph, relRay);
			const double dv = v_not_found ? POSITIVE_INFINITY : vertDist(pv, relRay);
			...
			if (rayExitsMapAtTransparentHit(
				wMap,
				m_player,
				relRay,
				vert,
			...
			const auto hitCell = [&]() {
				...
				if (render_internal_wall) {
					if (vert) {
						if (!(relRay >= m_player.deg90() && relRay < m_player.deg270())) {
							--c;
						}
					}
					else if (!(relRay >= m_player.deg180() && relRay < m_player.deg360())) {
						--r;
					}
				}
				else {
			...
			hits[hitCount++] = { mapKey, hitPoint, vert ? dv : dh, vert, hitCell };
		...
		for (size_t hi = hitCount; hi-- > 0; ) {
			...
			const bool effectiveInternalWall = render_internal_wall || hitOnPlayerCell;
			const auto face = wallFaceForHit(
				m_player,
				relRay,
				vert,
				effectiveInternalWall);
			const auto wallLayer = wMap.transparentWallTextureLayerAt(
				hitIt->cell.first,
				hitIt->cell.second,
				MapCell::transparentWallTexture(mapKey),
				face,
				effectiveInternalWall);
			...
			if (unsigned(k) < POSITIVE_INFINITY) {
				...
				if (currentCellRay >= 0 && currentCellRay < cellBound) {
					...
					if (wallHeight && !hitCameraCell) {
						transpShadingStretchBtl(
							ray,
							((m_player.getSlope() + m_player.getYProjRes()) >> 1) - ceilingProj - k,
							k,
							x_coord_source,
							0,
					...
					transpShadingStretchBtl(
						ray,
						((m_player.getSlope() + m_player.getYProjRes()) >> 1) - ceilingProj,
						k,
						x_coord_source,
						0,
						wMap.getCellDy(), //height
						...
						m_player.getYProjRes(),
						shadingAttr,
						wMap.getTexture(wallKey),
						TRANSP_COLOR,
						correctedDepth,
						wallLayer.overlay ? wMap.getTexture(wallLayer.overlay) : nullptr
					);
				} // if current_cell...
			} // if k...
		}
	...
	uint32_t height)
{
	...
	if (m_frameBuffer.width() != width ||
		m_frameBuffer.height() != height) {
		m_frameBuffer.resize(width, height);
	}

	if (m_columnDepthBuffer.width() != static_cast<uint32_t>(m_player.getXProjRes())) {
	...
	m_spritePixelDepthBuffer.assign(spriteDepthSize, ColumnDepthBuffer::farDepth());
	m_opaquePixelDepthBuffer.assign(spriteDepthSize, ColumnDepthBuffer::farDepth());
	...
	if (skyTexture) {
		skyTexture->fillBuffer(
			m_frameBuffer.data(),
			m_frameBuffer.width(),
			m_frameBuffer.height(),
			cameraRayOffset /*+ m_fps/30*/,
			org_x_res);
	}
	...
	for (int ray = rayBegin; ray < rayEnd; ++ray) {
		...
		const double floorScaledDistortLut = scaledDistortLut * playerHeightRatio;
		const double ceilScaledDistortLut =
			scaledDistortLut * (1.0 - playerHeightRatio);
		const double minWallDistance =
			std::max(1.0, std::min(wMap.getCellDx(), wMap.getCellDy()) / 16.0);
		const double renderDistance = std::max(hit.distance, minWallDistance);

		if (hit.distance > 0.0 && wallKey && !MapCell::isTransparentTexture(wallKey)) {
		...
		if (unsigned(k) < POSITIVE_INFINITY) {
			...
			for (int ceilRay = 0; ceilRay < (ceilBottom - ceilingProj); ++ceilRay) {
				...
				const Color c = texture->getPixel(xPicture % cellDx, yPicture % cellDy);
				...
				drawPixel32(
					ray,
					ceilRay,
					shadeWorldColor(c, std::min(1.0, shadingAttr) * m_brightness));
			}
				...
				++floorRay)
			{
				...
				if ((row<int(wMap.getRowCount())) && col<int(wMap.getColCount()) && row >= 0 && col >= 0) {
					const Cell mapKey = wMap[row][col];
					if (wMap.isSolidCellHit(row, col, mapKey)) continue;
			...
			if (wallKey && !MapCell::isTransparentTexture(wallKey)) {
				...
				if (shouldMirrorWallColumn(m_player, relRay, hit.vertical)) {
					currentCellRay = (cellBound - 1) - currentCellRay;
				}

				const double shadingAttr = double(k) / double(m_depthShadingPar);
				const int horizonY = ((m_player.getSlope() + m_player.getYProjRes()) >> 1);
				const int cellDy = wMap.getCellDy();
				...
				if (hit.block != nullptr) {
					for (const auto& span : hit.block->walls) {
						...
						const auto spanTextureLayer = wMap.textureLayerForWallSpanAt(
							hit.row,
							hit.column,
							span,
							hit.face);
						const auto spanTextureKey = spanTextureLayer.base;
						if (!spanTextureKey || MapCell::isTransparentTexture(spanTextureKey)) {
							continue;
						}

						const int spanTopWorld = span.top;
						const int spanBottomWorld = span.bottom;
						const int spanTopScreen = horizonY + centerProj - int((double(k) * spanTopWorld) / cellDy);
						const int spanBottomScreen = horizonY + centerProj - int((double(k) * spanBottomWorld) / cellDy);
						...
						shadingStretchBtl(
							...
							m_player.getYProjRes(),
							shadingAttr,
							wMap.getTexture(spanTextureKey),
							correctedDepth,
							spanTextureLayer.overlay ? wMap.getTexture(spanTextureLayer.overlay) : nullptr);

						if (spanBottomWorld >= cellDy) {
							// span starts above the floor: ceiling slice visible beneath top of span
								...
								++ceilRay)
							{
								...
								const auto ceilKey = MapCell::ceilingTexture(mapKey);
								if (MapCell::isTransparentTexture(ceilKey)) {
									continue;
								}
								...
								const double shadingC = m_ceilFloorShadingPar / double(distToPtOnCeiling);
								const Color c = texture->getPixel(xPicture % cellDx, yPicture % cellDy);
								const auto depthIndex = static_cast<size_t>(ceilRay)
									* static_cast<size_t>(m_player.getXProjRes())
									+ static_cast<size_t>(ray);
								...
								drawPixel32(
									ray,
									ceilRay,
									shadeWorldColor(c, std::min(1.0, shadingC) * m_brightness));
							}
						}
				...
				if (!renderedAnyExtraSpan) {
					...
					if (wallHeight) {
							...
							++ceilRay)
						{
							...
							if (wMap.isSolidCellHit(row, col, mapKey)) continue;
							const auto ceilKey = MapCell::ceilingTexture(mapKey);
							if (MapCell::isTransparentTexture(ceilKey)) continue;
							const auto texture = wMap.getTexture(ceilKey);
							if (!texture) continue;
	...
	m_lastProfile.spritesMs = ProfileMs(profileSprites1 - profileWalls1).count();
	m_lastProfile.transparentWallsMs =
		ProfileMs(profileTransp1 - profileSprites1).count();
	m_lastProfile.viewWeaponMs = ProfileMs(profileWeapon1 - profileTransp1).count();
