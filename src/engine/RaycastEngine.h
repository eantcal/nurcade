...
{
public:
    enum class SpriteCollisionMode {
        PassThroughWalls,
        BlockSolidWalls
    };
    ...
    void setBrightness(double brightness) noexcept {
        m_brightness = std::clamp(brightness, 0.05, 2.0);
    }
    ...
    const std::vector<Sprite>& sprites() const noexcept {
        return m_sprites;
    }
    ...
    Sprite* sprite(size_t spriteIndex) noexcept {
        return spriteIndex < m_sprites.size() ? &m_sprites[spriteIndex] : nullptr;
    }
...
private:
    ...
    void drawAlphaPixel32(unsigned int x, unsigned int y, Color color_value) noexcept {
        ...
        drawPixel32(
            ...
            y,
            makeColor(
                (colorRed(color_value) * alpha + colorRed(dest) * invAlpha) / 255,
                (colorGreen(color_value) * alpha + colorGreen(dest) * invAlpha) / 255,
                (colorBlue(color_value) * alpha + colorBlue(dest) * invAlpha) / 255));
    }

    void renderTranspWall(
        WorldMap& aMap,
