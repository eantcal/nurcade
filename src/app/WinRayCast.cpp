...
// object (editable in WinRaycastEditor), read into WorldMap by WorldJsonLoader.
...
TCHAR g_szAppTitle[] = "WinRayCast Player";
...
namespace {
	...
	constexpr const char* kAboutText =
		"WinRayCast Player by antonino.calderone@gmail.com\r\n"
	...
	bool looksLikeWorldJson(const std::string& path)
	{
		...
		return text.find("\"format\"") != std::string::npos
			&& text.find("\"winraycast.world\"") != std::string::npos;
	}
	...
	std::string leaderboardFilePath()
	{
		...
		const auto base = localAppData != nullptr && *localAppData != '\0'
			...
			: g_currentWorldDir;
		const auto directory = joinPath(base, "WinRayCast");
...
bool OpenProjectFromMenu(HWND hWnd)
{
	...
	openFile.lpstrFilter =
		"WinRayCast project/world (*.winrayproj.json;*.world.json;*.json)\0*.winrayproj.json;*.world.json;*.json\0"
		"All files (*.*)\0*.*\0";
	openFile.lpstrFile = fileName;
	openFile.nMaxFile = MAX_PATH;
	openFile.Flags = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST | OFN_NOCHANGEDIR;
	openFile.lpstrTitle = "Open WinRayCast Project or World";
