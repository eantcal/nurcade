...
{
    const auto value =
        static_cast<DWORD>((clampVolumePercent(volumePercent) * 0xFFFF) / 100);
...
std::wstring widenPath(const std::string& path)
{
    ...
    if (length == 0) {
        ...
        length = MultiByteToWideChar(
            codePage, 0, path.c_str(), -1, nullptr, 0);
    }
...
bool ensureMediaFoundationStarted(std::string* error)
{
    ...
    if (error != nullptr) {
        *error = "Could not start Media Foundation: " + hresultText(startupResult);
    }
    return false;
}

bool decodePcmWithMediaFoundation(
    ...
    std::string* error)
{
    ...
    auto result = MFCreateSourceReaderFromURL(widePath.c_str(), nullptr, &reader);
    if (FAILED(result)) {
        if (error != nullptr) {
    ...
    if (FAILED(result)) {
        if (error != nullptr) {
            *error = "Could not create audio output type: " + hresultText(result);
        }
        cleanupCom();
        return false;
    }
    ...
    std::string* error)
{
    ...
    sound.format.nBlockAlign =
        static_cast<WORD>((sound.format.nChannels * sound.format.wBitsPerSample) / 8);
    ...
    std::string* error)
{
    ...
    return decodePcmWithMediaFoundation(
        path,
        sound.bytes,
        sound.format,
        error);
...
std::shared_ptr<const DecodedPcmSound> cachedPcmSound(
    const std::string& path)
{
...
std::shared_ptr<const DecodedPcmSound> cachePcmSound(
    const std::string& path,
    std::shared_ptr<const DecodedPcmSound> sound)
{
    std::lock_guard<std::mutex> lock(g_pcmSoundCacheMutex);
    const auto item = g_pcmSoundCache.find(path);
    ...
    return queueBackgroundTask(
        [path, extension, volumePercent]() {
            ...
            if (sound == nullptr) {
                ...
                if (!decodeSoundToPcm(path, extension, *decoded, &ignoredError)) {
                    return;
                }
            ...
            std::string ignoredError;
            playPcmOnce(std::move(bytes), sound->format, volumePercent, &ignoredError);
        },
        error);
}
    ...
    std::string* error)
{
    ...
    return playPcmOnce(std::move(sound.bytes), sound.format, volumePercent, error);
}
}
