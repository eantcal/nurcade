...
public:
	...
	// Draw the 3D frame buffer (its top-left srcWidth x srcHeight region) scaled
	// into destRect on screen, GPU-accelerated with linear filtering.
	void draw3D(const FrameBuffer& frame,
		const RECT& destRect,
		int srcWidth,
