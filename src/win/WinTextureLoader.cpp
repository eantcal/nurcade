...
std::shared_ptr<Texture> loadTextureFromBitmap(HDC hdc,
    HBITMAP hBitmap,
    int width,
    int height)
{
    ...
    const auto copiedLines = GetDIBits(
        textureHdc,
        hBitmap,
        0,
        height,
    ...
    int height)
{
    ...
    const auto targetWidth =
        width > 0 ? static_cast<UINT>(width) : sourceWidth;
